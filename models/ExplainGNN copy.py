...
class Explainer(nn.Module):
    ...
    def explain(self, h, node_edge, label_edge):
        ...
        edge_matrix = edge_feature_n @ edge_feature_l.T
...
class ExplainGNN(nn.Module):
    ...
    def basline(self, features, edge_index, edge_weight, label_nodes, node, G, K=1):
        ...
        print(features[node[0]].shape)
        print(features[label_nodes[0]].norm(dim=1).shape)
        score = features[node[0]] @ features[label_nodes[0]].T/ \
                (features[node[0]].norm(dim=1, keepdim=True) @ (features[label_nodes[0]].norm(dim=1, keepdim=True).T))
        ...
        for i in range(K):
            ...
            edge_matrix = edge_feature_n @ edge_feature_l.T/\
                        (edge_feature_n.norm(dim=1, keepdim=True)@edge_feature_l.norm(dim=1, keepdim=True).T)
    ...
    def explain(self, features, edge_index, edge_weight, label_nodes, node, G, K=1):
        ...
        self.eval()
        score = features[node[0]] @ features[label_nodes[0]].T#self.predict(features, edge_index, edge_weight, label_nodes, node, train_phase=False)
        _, indices = score.squeeze().topk(K)
        # indices = label_nodes[0][indices]
        print(indices)
        print(node[0])
        ...
        for i in range(K):
            ...
            label_edge = label_nodes[3][:,label_nodes[4]==indices[i]]
            pair_edge = self.explainer.explain(features, node[3],label_edge)
            edge_acc += self.check_edge_type(G, node[3], pair_edge)
    ...
    def predict(self, features, edge_index, edge_weight, label_nodes, nodes, train_phase, K=None):
        ...
        node_score = embedding[nodes[0]] @ embedding[label_nodes[0]].T
        if train_phase:
    ...
    def fit(self, features, edge_index, edge_weight, labels, label_nodes, val_nodes, train_iters=200, verbose=False):
        """
        """
        ...
        self._train_with_val(label_nodes, val_nodes, train_iters, verbose)

    def _train_with_val(self, label_nodes, val_nodes, train_iters, verbose):
        if verbose:
        ...
        for i in range(train_iters):
            ...
            acc_val = self.test(label_nodes,val_nodes)
    ...
    def test(self, label_nodes, nodes):
        ...
        preds = score @ train_labels
        acc_val = utils.accuracy(preds, self.labels[nodes[0]])
