...
parser.add_argument('--K', type=int, default=-1,
                    help='trianing nodes')
parser.add_argument('--pred_K', type=int, default=15,
                    help='K')
...
model.fit(data.x, data.edge_index, data.edge_attr, data.y, train_mask, val_mask, train_iters=args.epochs, verbose=True)
...
for i in range(len(test_loader)):
    ...
    node_acc, edge_acc, nb_edge = model.explain(data.x, data.edge_index, data.edge_attr, label_nodes, test_loader[i], K=1)
    acc += node_acc
    print("edge: {:4f}, node: {:.4f}, {}".format(edge_acc, edge_acc, nb_edge))
